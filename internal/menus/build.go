...
// Build assembles the full menu bar. Each menu is constructed by hand
// so layout (order, separators, submenus) is explicit and inspectable.
func Build(bounds geom.Rect, reg *commands.Registry) *fvmenus.MenuBar {
	bar := fvmenus.NewMenu(
		&fvmenus.Item{Name: "~F~ile", Sub: fileMenu(reg)},
		&fvmenus.Item{Name: "~E~dit", Sub: editMenu(reg)},
		&fvmenus.Item{Name: "~V~iew", Sub: viewMenu(reg)},
		...
		&fvmenus.Item{Name: "~T~ransfer", Sub: transferMenu(reg)},
		&fvmenus.Item{Name: "~H~elp", Sub: helpMenu(reg)},
	)
	return fvmenus.NewMenuBar(bounds, bar)
}

func fileMenu(reg *commands.Registry) *fvmenus.Menu {
	return menu(
...
func viewMenu(reg *commands.Registry) *fvmenus.Menu {
	return menu(
...
func transferMenu(reg *commands.Registry) *fvmenus.Menu {
	return menu(item(reg, commands.CmdSFTPBrowser))
}
